...
fn main() {
    ...
    let mut seats = Vec::<(String, Option<(wl_keyboard::WlKeyboard, calloop::Token)>)>::new();
        ...
        if let Some((has_kbd, name)) = sctk::seat::with_seat_data(&seat, |seat_data| {
            ...
            if has_kbd {
                ...
                match map_keyboard_repeat(
                    ...
                    Ok((kbd, repeat_token)) => {
                        seats.push((name, Some((kbd, repeat_token))));
                    }
                    Err(e) => {
    ...
    let _seat_listener = env.listen_for_seats(move |seat, seat_data, _| {
        ...
        if seat_data.has_keyboard && !seat_data.defunct {
            ...
            if let Some((kbd, kbd_token)) = opt_kbd.take() {
